...
int main(int argc,char *argv[])
{
	...
	if ((len_1 > 10) || (len_2 > 10) || (len_1 == 0 ) || (len_2 == 0)) 
		raler("%s %s %s , pas plus grand que 10 caracter  es et non vide",argv[0],argv[1],argv[2]);
	...
	if (arg2_nombre == 1) {
		...
		if (placer_invit == -1) 
			...
			if(c->groupes[x].membres_present == c->groupes[x].nb_membres_gr) {
